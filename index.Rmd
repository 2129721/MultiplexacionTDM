...
# Definir las señales a multiplexar
signal1 <- cos(10 * pi * t)  # cos(10πt)
signal2 <- cos(20 * pi * t)  # cos(20πt)
signal3 <- (4/pi) * sin(6 * pi * t) + (1/3) * sin(18 * pi * t)  # combinación de senos

# Crear una señal cuadrada que sirva de máscara para la multiplexación
x <- seq(0, 2 * 10 * 2 * pi, length.out = 1000)
sq <- square(x, duty = 33)  # 33% duty cycle para alternar entre las señales
sq <- 0.5 * (1 + sq)

# Crear una señal multiplexada en el tiempo usando la máscara cuadrada
multiplexed_signal <- numeric(length(t))
multiplexed_signal[sq == 1] <- signal1[sq == 1]
multiplexed_signal[sq == 0] <- signal2[sq == 0]

# Alternar las señales en diferentes partes del tiempo
sq2 <- square(x, duty = 66)  # Segunda máscara para la tercera señal
multiplexed_signal[sq2 == 0 & sq == 1] <- signal3[sq2 == 0 & sq == 1]

# Graficar las señales originales y la multiplexada
highchart() %>%
  hc_add_series(cbind(t, signal1), name = "Señal 1") %>%
  hc_add_series(cbind(t, signal2), name = "Señal 2") %>%
  hc_add_series(cbind(t, signal3), name = "Señal 3") %>%
  hc_add_series(cbind(t, multiplexed_signal), name = "Señal Multiplexada", color = "yellow") %>%
  hc_title(text = "Multiplexación de señales en el dominio del tiempo") %>%
  hc_xAxis(title = list(text = "Tiempo")) %>%
  hc_yAxis(title = list(text = "Amplitud"), min = -2, max = 2) %>%
